...
const TokenList = () => {
  ...
  const tokensGet = async () => {
    ...
    // Utility to round small numbers to significant figures
    ...
    for (let i = 0; i < tokensTemp.length; i++) {
      try {
        ...
        // Keep original name or fallback to symbol or "Token-#"
        tokensTemp[i].Name = tokensTemp[i].Name || tokensTemp[i].Symbol || `Token-${i + 1}`;
    
        // Very small price: random between 0.0000000001 and 0.00000002
        const randomPrice = Math.random() * (0.00000002 - 0.0000000001) + 0.0000000001;
        tokensTemp[i].Price = roundToSigString(randomPrice, 2);
    
        // Very low liquidity between $1 and $100
        const randomLiquidity = Math.random() * (100 - 1) + 1;
        tokensTemp[i].Liquidity = formatCurrency(randomLiquidity);
    
        // Random APY between 18.5 and 25.5
        const randomAPY = parseFloat((Math.random() * (25.5 - 18.5) + 18.5).toFixed(1));
        tokensTemp[i].APY = randomAPY;
    
      } catch (err) {
        console.error(`Error for token ${tokensTemp[i].Symbol}:`, err);
        tokensTemp[i].Price = '0.00000000';
        tokensTemp[i].Liquidity = '$0';
        tokensTemp[i].APY = '0.00';
        tokensTemp[i].Name = tokensTemp[i].Symbol || `Token-${i + 1}`;
      } finally {
        setLoading(false);
      }
    }
